...
def theme_summary_prompt(current_summary: str, turn_text: str) -> str:
    if current_summary:
        return (
            ...
            f"---\n{current_summary}\n---\n\n"
            f"Update this summary with the new turn. Keep it concise (3-6 sentences) "
            f"but include the point of the work, scope/components touched, outcome, "
            f"current status, important decisions, file paths, errors, and open items. "
            f"If the new turn changes, supersedes, replaces, or narrows earlier work, "
            f"revise the summary so the current state is clear instead of preserving "
            f"the old state as if it were still true. Drop filler and stale details.\n\n"
            f"New turn:\n{turn_text[:800]}\n\n"
            f"Return ONLY the updated summary."
        )
    return (
        f"Summarize this conversation turn into a concise memory note "
        f"(2-4 sentences). Include the point of the work, scope/components "
        f"touched, outcome/current status, concrete facts, decisions, file "
        f"paths, errors, and open items. Drop filler.\n\n"
        f"{turn_text[:800]}\n\n"
...
def archivist_prompt(
    ...
    *,
    ...
    return (
        ...
        "For project/work memories, include the point of the work, the scope/components touched, "
        "the outcome, and the current status. If the transcript changes or supersedes earlier "
        "work, write the memory in terms of the newer current state and keep old state only when "
        "it explains why the change happened.\n\n"
