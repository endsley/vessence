"""Benchmark the initial ack LLM with varying tree index sizes.

Measures: response latency, token usage (approx), classification quality.
Tests both compact path and flat list formats at 50/100/200/300 leaf scales.

Usage:
    python benchmark_initial_ack.py [--model haiku|sonnet|opus] [--rounds 3]
"""
...
# Add vessence to path
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
from agent_skills.claude_cli_llm import (
    completion_utility,
    completion_agent,
...
# Sample user prompts of varying complexity
TEST_PROMPTS = [
    {
        "id": "simple_recall",
...
def run_single_benchmark(
    ...
    try:
        response = completion(full_prompt, model=model, max_tokens=1024, timeout=timeout)
...
def evaluate_classification(result: dict, expected_branches: list[str]) -> dict:
...
def run_full_benchmark(model: str, rounds: int = 3) -> dict:
    """Run the full benchmark suite."""
    leaf_counts = [50, 100, 200, 300]
    formats = ["compact", "flat"]
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark initial ack with tree indexes")
    parser.add_argument(
        "--model",
        choices=["haiku", "sonnet", "opus"],
        default="haiku",
        help="Model tier to benchmark (default: haiku)",
    )
    parser.add_argument(
        "--rounds",
        type=int,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON file (default: results_<model>.json)",
    )
    ...
    model_map = {
        "haiku": CHEAP_MODEL,
        "sonnet": SMART_MODEL,
        "opus": os.environ.get("BRAIN_HEAVY_CLAUDE", "claude-3-6-opus-20260320"),
    }
    model = model_map[args.model]

    print(f"Benchmarking initial ack LLM")
    print(f"Model: {args.model} ({model})")
    print(f"Rounds per test: {args.rounds}")
    print(f"Test prompts: {len(TEST_PROMPTS)}")

    results = run_full_benchmark(model, args.rounds)

    print_summary(results)

    # Save full results
    output_file = args.output or os.path.join(
        BENCHMARK_DIR, f"results_{args.model}.json"
    )
