"""Benchmark gemma4:e2b on weather routing WITHOUT injecting weather data.

Same 100 prompts as the context variant, but the model is only asked:
"is this a weather question the system should try to answer from its
local weather cache?"

This removes the 2,000-char JSON from every request — so we expect
noticeably lower latency and a different accuracy profile. Without the
data the model has no way to distinguish "field we store" vs "field we
don't" (dew point, barometric pressure, etc.), so the no_unanswerable
category is inherently ambiguous here and may score higher than ideal.
"""
...
SYSTEM_TEMPLATE = """You are a routing classifier. Given a user prompt, \
output a confidence score between 0.00 and 1.00 for whether the prompt is \
a request for CURRENT or UPCOMING weather conditions in the user's local \
area (temperature, humidity, wind, precipitation, forecast, air quality, \
UV, etc.) that a local weather cache would typically serve.

Scoring guidance:
- 1.00 = clearly a request for current or forecast weather in the user's \
local area.
- 0.00 = casual small talk about the weather that is not a question, OR \
a question about the code / implementation of the weather feature, OR a \
request about weather in a different specific city, OR a request about \
past weather.
- Use intermediate values when uncertain.

This is a ROUTING score: you are deciding whether a weather lookup should \
be invoked, not whether you personally know the answer.
...
SCORE_RE = re.compile(r"\b(0?\.\d+|1(?:\.0+)?|0(?:\.0+)?)\b")


def classify(prompt: str) -> tuple[float, float, str]:
    body = {
        "model": MODEL,
        "prompt": SYSTEM_TEMPLATE.format(prompt=prompt),
        "stream": False,
        "think": False,
...
def main() -> None:
    ...
    out_path = Path(__file__).parent / "benchmark_gemma_weather_class_nocontext_results.json"
    out_path.write_text(
        json.dumps(
            {
                "model": MODEL,
