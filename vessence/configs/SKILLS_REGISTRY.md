...
-   **Capability:** Memory Retrieval & Librarian
    ...
    -   **Codex CLI installer:** `$VESSENCE_HOME/startup_code/install_codex_memory.py` — idempotently writes `~/.codex/hooks/jane_memory_hook.py`, `~/.codex/jane-memory-instructions.md`, lifecycle hooks, and both `jane-memory` and `jane-coordination` MCP registrations in `~/.codex/config.toml`

-   **Capability:** Multi-Codex Code Coordination
    -   **Board CLI:** `$VESSENCE_HOME/agent_skills/code_coordination.py` — posts tasks, atomically claims scoped files or directory trees, reports conflicts, sends messages, heartbeats leases, and releases completed/stale work
    -   **Board storage:** `$VESSENCE_DATA_HOME/coordination/code_board.db` — SQLite WAL database supporting concurrent sessions without a configured agent-count limit; stale leases default to four hours
    -   **Codex MCP bridge:** `$VESSENCE_HOME/startup_code/codex_coordination_mcp.py` — exposes board, task, claim, release, heartbeat, message, and finish tools as `jane-coordination`
    -   **Legacy exclusive gate:** `$VESSENCE_HOME/agent_skills/code_lock.py` — reserved for repository-wide operations and waits for active scoped claims before acquiring exclusivity
...
-   **Skill: Rheumatoid Arthritis Remission Research Loop**
    ...
    -   **What it does:** Runs daily at 2 PM ET as a dedicated RA literature researcher for Kathia's remission/asymptomatic-state goal. Searches PubMed/PMC plus core guideline sources, saves every processed source/paper to `$VAULT_HOME/research/rheumatoid_arthritis_remission/papers/`, caches per-source summaries in `$VAULT_HOME/research/rheumatoid_arthritis_remission/summaries/`, stores raw run caches in `$VESSENCE_DATA_HOME/research/rheumatoid_arthritis_remission/cache/`, and regenerates both `$VAULT_HOME/research/rheumatoid_arthritis_remission/recommendations/recommendation_plan.md` and `$VAULT_HOME/research/rheumatoid_arthritis_remission/ra_remission_recommendation_scheme.md`.
    -   **Smart synthesis:** Each run invokes Codex via `jane.automation_runner.run_automation_prompt(provider="codex")` as the high-judgment synthesis pass. Codex re-reads the compressed context and cached summaries, restates the mission to help Kathia reach sustained remission/asymptomatic state, writes a new compressed context for the next run, documents discoveries, and combines all research into actionable categories: at-home actions, tracking, tests to discuss, food/diet, lifestyle, medical strategy questions, and emerging technology/neuromodulation. Local Ollama is only a fallback.
    -   **Reports:** Sends the initial app/email report after 4 runs, then every 72 hours. Reports are also rendered as patient-friendly shareable HTML with a plain-English bottom line, action checklist, vocabulary/concept explainer, evidence cards, clinician brief, and unguessable public share link for sharing with ChatGPT. Scheduled cron must not pass `--send-report-now`; that flag is manual-only.
    -   **Safety boundary:** Research support only. Medication, biologic/JAK inhibitor, steroid, supplement, or treatment changes require Kathia's rheumatologist.
