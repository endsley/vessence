...
- **Description:** Performs system-level cleanup: removes temp files, rotates oversized logs, and deletes log files older than 14 days from the runtime logs tree.
...
## 12. Autonomous Prompt Queue Runner
- **Schedule:** `*/5 * * * *` (Runs every 5 minutes)
- **Script Path:** `$VESSENCE_HOME/agent_skills/prompt_queue_runner.py`
- **Description:** When Chieh has been idle for 5+ minutes, picks the next uncompleted item from `vault/documents/prompt_list.md`, runs it via the shared automation runner, sends progress and results to Discord, marks the prompt `[COMPLETE]` or `[INCOMPLETE]`, and logs to short-term memory. Pauses mid-run if Chieh becomes active. Skips entirely if not idle.

## 13. Ambient Task Research
- **Schedule:** `*/30 * * * *` (Runs every 30 minutes)
- **Script Path:** `$VESSENCE_HOME/agent_skills/ambient_task_research.py`
- **Description:** When Chieh has been idle for 10+ minutes, extracts all unchecked tasks (`- [ ]`) from the Project Ambient spec progress tracker (all phases), searches DuckDuckGo for how others have implemented each one, synthesizes findings with qwen2.5-coder:14b into actionable implementation notes, and caches results in `ambient_task_research_cache.json` (7-day TTL per task). Processes up to 5 tasks per run. Sends a Discord summary of what was researched.
...
- **Description:** Fetches news for all tracked topics via Google News RSS + article scraping, generates brief/full summaries via gemma3:12b, downloads images, generates TTS audio via Docker XTTS (GPU), saves briefing to latest_briefing.json. Also triggered manually via refresh button on web.
...
## 19. Ambient Task Research — REMOVED (2026-03-24)
- **Previously:** `0 6 * * *` — removed from crontab per Chieh's request.

## 20. Evolving Code Map Keywords (NEW 2026-03-25)
- **Schedule:** `10 2 * * *` (Daily at 2:10 AM)
