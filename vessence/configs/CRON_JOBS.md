...
- **Description:** Single entry point that runs ALL nightly self-improvement jobs sequentially with per-job time budgets. Currently dispatches: (1) `doc_drift_auditor.py` — compares each `configs/*.md` registry against actual filesystem/cron state, auto-fixes safe drifts, commits with `auto-doc-sync:` prefix; (2) `nightly_code_auditor.py` — picks one module from `configs/auditable_modules.md`, generates tests via Opus, attempts up to 3 fixes, commits if green; (3) `pipeline_audit_100.py --n 30` — runs 30 real user prompts through the live pipeline, auto-fixes Stage 1 'others' misclassifications by adding exemplars to ChromaDB; (4) `dead_code_auditor.py` — scans for unreferenced files, dead functions, and duplicate function bodies. Conservative auto-deletion: ONLY removes files in `agent_skills/` or `test_code/` that are >60 days old, <500 lines, with zero references in the entire tree. Everything else is logged to `configs/dead_code_report.md` for human review. Auto-deletes commit with `auto-dead-code:` prefix. Each job has its own log + timeout. Add new jobs by appending to the JOBS list in the script. Summary written to `configs/self_improve_log.md` after every run. The final reporting step writes `configs/self_improvement_latest.md`, an easy-to-read stage-by-stage report covering what each stage did, problems found, improvements made, and evidence files; timestamped copies are archived under `$VESSENCE_DATA_HOME/reports/self_improvement/`.
